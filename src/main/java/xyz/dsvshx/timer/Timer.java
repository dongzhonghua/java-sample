 ...
 * repeated execution at regular intervals一定周期内.
 *
 ...
 * completes.
 * 单个线程依序来执行所有的任务，计时任务应是非常快的任务。如果计时任务花费过多时间来完成，
 * 它将“占用”计时器的任务执行线程。反过来，这可能会延迟后续任务的执行，
 * 这可能会“累加”并在有问题的任务最终完成时（如果有）迅速连续执行。
 *
 ...
 * {@code Timer}.
 * <p>
 * ScheduledThreadPoolExecutor相当于是timer的升级，可以支持多个线程来运行任务，如果是一个线程的则等效于timer
 * <p>
 * TODO 这个怎么变成多个任务同时执行呢？
...
public class Timer {
     ...
     * <p>
     * 该数据结构与timerThread共享。计时器通过其各种调度调用产生任务，
     ...
     * @since 1.5
     */
    // 这个方法大多数情况下不用，在需要移除过期任务的时候才需要，这个不会影响定时器的执行，只会删除过期任务。
    public int purge() {
        int result = 0;
...
class TaskQueue {
     ...
     * each node n in the heap, and each descendant后代 of n, d,
    ...
    void add(TimerTask task) {
        // Grow backing store if necessary，扩容
    ...
    void removeMin() {
        queue[1] = queue[size];
        queue[size--] = null;  // Drop extra reference to prevent memory leak,这个做法值得借鉴
        fixDown(1);
    }
